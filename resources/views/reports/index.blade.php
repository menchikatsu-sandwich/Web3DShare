...
@section ('content')
    <div class="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4 backdrop-blur-md">
        ...
        <div
            class="flex h-full max-h-[88vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-neon/20 bg-darkPanel text-gray-200 shadow-[0_0_40px_rgba(0,255,136,0.08)]"
        >
            ...
            <div class="min-h-0 flex-1 space-y-4 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-gray-800">
                <div class="inline-flex rounded-xl border border-gray-800 bg-black/40 p-1 shadow-inner backdrop-blur-sm">
                    <button
                        type="button"
                        onclick="switchReportsPanel('reports')"
                        id="panel-tab-reports"
                        class="reports-panel-tab flex items-center gap-2 rounded-lg border border-transparent px-4 py-2 text-xs font-bold tracking-wider uppercase transition-all duration-300"
                    >
                        <span>Reports</span>
                ...
                <section id="panel-model-notices" class="reports-panel hidden space-y-3">
                        <div class="flex items-end justify-between gap-4">
                            ...
                            <span class="rounded-full border border-blue-500/20 bg-blue-500/10 px-2.5 py-1 text-xs font-bold text-blue-300">
                                {{ $ownerReports->count() }}
                            </span>
                        </div>
                        ...
                        @forelse ($ownerReports as $ownerReport)
                            ...
                            @endphp
                            <article class="rounded-xl border border-blue-500/20 bg-blue-500/10 p-5">
                                <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <p class="mt-3 whitespace-pre-wrap text-sm leading-relaxed text-blue-50">{{ $ownerReport->owner_message }}</p>
                                        <p class="mt-3 text-[10px] font-bold uppercase tracking-widest text-blue-300/80">
                                    ...
                                    @if ($ownerModelAvailable)
                                        <a
                                            ...
                                            class="inline-flex shrink-0 items-center justify-center rounded-lg border border-blue-400/30 bg-blue-400/10 px-4 py-2 text-sm font-semibold text-blue-100 transition-colors hover:bg-blue-400/20"
                                        >
                                            Edit Model
                                        </a>
                                    @else
                                        <span class="inline-flex shrink-0 items-center justify-center rounded-lg border border-red-500/20 bg-red-500/10 px-4 py-2 text-sm font-semibold text-red-200">
                                            Model unavailable
                                        </span>
                                    @endif
                                </div>
                ...
                <section id="panel-reports" class="reports-panel space-y-4">
                    <div class="mb-2 inline-flex rounded-xl border border-gray-800 bg-black/40 p-1 shadow-inner backdrop-blur-sm">
                        <button
                            type="button"
                ...
                @forelse ($reports as $report)
                    ...
                    <article
                        ...
                        class="report-card rounded-xl border border-gray-800 bg-darkBg p-5 transition-colors hover:border-neon/30"
                    >
                        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                            <div class="min-w-0 flex-1">
                                <div class="flex flex-wrap items-center gap-2">
                        ...
                        @if ($description)
                            <div class="mt-4 space-y-3">
                                ...
                                @if ($replyText)
                                    <div class="rounded-lg border border-green-300 bg-green-50 p-3 text-xs text-gray-700 dark:border-neon/20 dark:bg-neon/5 dark:text-gray-300">
