...
>
    <div
        data-tour-welcome
        class="fixed inset-0 hidden items-center justify-center bg-black/80 p-4 backdrop-blur-md"
        style="z-index: 300"
    >
        <div
        ...
        >
            ...
            <div class="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
                <button
                    type="button"
                    data-tour-start
                    class="inline-flex justify-center rounded-xl bg-green-500 px-7 py-3 font-bold text-white shadow-lg transition-all hover:bg-green-600 dark:bg-neon dark:text-black dark:hover:bg-[#00cc6a]"
                >
                    Get Started
                </button>
                <button
                    type="button"
                    data-tour-welcome-skip
                    class="text-sm font-semibold text-gray-500 transition-colors hover:text-green-600 dark:text-gray-400 dark:hover:text-neon"
                >
                    Skip tutorial
                </button>
            </div>
    ...
    <div data-tour-stage class="pointer-events-none fixed inset-0 hidden" style="z-index: 300">
        ...
        >
            <div class="flex items-start justify-between gap-4">
                <div>
                    <p data-tour-count class="mb-2 text-[11px] font-bold tracking-[0.2em] text-green-600 uppercase dark:text-neon"></p>
                    <h3 data-tour-title class="text-xl leading-snug font-bold text-gray-950 dark:text-white"></h3>
                </div>
                <button
                    type="button"
                    data-tour-close
            ...
            <div class="mt-5 grid grid-cols-2 gap-3">
                <button
                    type="button"
                    data-tour-back
                    class="rounded-xl border border-gray-200 bg-gray-50 px-4 py-2.5 font-semibold text-gray-700 transition-all hover:border-green-400 dark:border-gray-800 dark:bg-darkBg dark:text-gray-300 dark:hover:border-neon/40"
                >
                    Back
                </button>
                <button
                    type="button"
                    data-tour-next
                    class="rounded-xl bg-green-500 px-4 py-2.5 font-bold text-white transition-all hover:bg-green-600 dark:bg-neon dark:text-black dark:hover:bg-[#00cc6a]"
                >
                    Next
                </button>
            </div>
            <button
                type="button"
                data-tour-skip
                class="mt-4 w-full text-center text-sm font-semibold text-gray-500 transition-colors hover:text-green-600 dark:text-gray-400 dark:hover:text-neon"
            >
                Skip tutorial
            </button>
        </section>
    ...
    (() => {
        ...
        function routeMatches(route) {
            ...
            if (
                url.pathname === '/' &&
                !url.search &&
                new URLSearchParams(window.location.search).get('filter') === 'my_models'
            ) {
                return false;
            }
        ...
        function waitForModelViewerReady(target, timeout = 18000) {
            ...
            const loaded =
                viewer.loaded === true || viewer.modelIsVisible === true || viewer.getAttribute('loaded') === 'true';
        ...
        function attachSpotlightListeners() {
            ...
            if (window.MutationObserver && activeTarget) {
                const observedRoot =
                    activeTarget.closest('[data-model-shell]') || activeTarget.parentElement || document.body;
