    ...
    <div class="flex-1 w-full bg-white dark:bg-darkPanel border border-gray-200 dark:border-neon/10 shadow-md dark:shadow-none rounded-2xl p-6 lg:p-8 min-h-[600px] transition-colors duration-300">
        ...
        <div id="models" class="tab-content">
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($models as $m)
                <div class="bg-gray-50 dark:bg-darkBg border border-gray-200 dark:border-gray-800 p-3 rounded-xl overflow-hidden hover:border-green-300 dark:hover:border-neon/30 transition-colors group shadow-sm">
                    ...
                    <h3 class="font-semibold text-gray-800 dark:text-gray-200 truncate">{{ $m->title }}</h3>
                    <p class="text-xs text-gray-500 mb-4">By: <span class="text-green-600 dark:text-neon">{{ $m->user->username }}</span></p>
                    <form method="POST" action="/admin/delete-model/{{ $m->id }}">
                        @csrf @method('DELETE')
                        <button class="w-full bg-red-100 dark:bg-red-500/10 text-red-600 dark:text-red-500 hover:bg-red-500 hover:text-white py-2 rounded-lg text-sm font-medium transition-colors border border-red-200 dark:border-transparent">Take Down</button>
                    </form>
                </div>
                @endforeach
            </div>
        ...
        <div id="users" class="tab-content">
            ...
            <div class="space-y-3">
                @foreach($users as $u)
                <div class="flex flex-col md:flex-row md:items-center justify-between bg-gray-50 dark:bg-darkBg border border-gray-200 dark:border-gray-800 p-4 rounded-xl hover:border-gray-300 dark:hover:border-gray-700 transition-colors shadow-sm">
                    <div class="mb-4 md:mb-0 flex items-center gap-3">
                        ...
                        <div>
                            <p class="font-semibold text-gray-800 dark:text-gray-200">{{ $u->username }}</p>
                            <span class="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full {{ $u->role === 'admin' ? 'bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400' : ($u->role === 'moderator' ? 'bg-blue-100 dark:bg-blue-500/20 text-blue-700 dark:text-blue-400' : 'bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300') }}">{{ $u->role }}</span>
                        </div>
                    </div>
                    <div class="flex gap-2">
                        ...
                        <form method="POST" action="/admin/promote/{{ $u->id }}">
                            @csrf
                            <button class="bg-gray-200 dark:bg-gray-800 hover:bg-green-100 dark:hover:bg-green-500/20 text-gray-700 dark:text-gray-300 hover:text-green-700 dark:hover:text-neon px-4 py-2 rounded-lg text-sm font-medium transition-colors border border-gray-300 dark:border-transparent">Make Mod</button>
                        </form>
                        @endif
                        @if($u->id !== auth()->id())
                        <form method="POST" action="/admin/delete-user/{{ $u->id }}">
                            @csrf @method('DELETE')
                            <button class="bg-gray-200 dark:bg-gray-800 hover:bg-red-100 dark:hover:bg-red-500/20 text-gray-700 dark:text-gray-300 hover:text-red-600 dark:hover:text-red-400 px-4 py-2 rounded-lg text-sm font-medium transition-colors border border-gray-300 dark:border-transparent">Ban User</button>
                        </form>
                        @endif
                    </div>
                </div>
...
<script>
    function changeTab(tabId, btnElement) {
        ...
        document.querySelectorAll('.tab-btn').forEach(btn => {
            // Hapus style active (baik terang maupun gelap)
            btn.classList.remove('text-green-700', 'bg-green-100', 'text-neon', 'bg-neon/10', 'dark:text-neon', 'dark:bg-neon/10');
            // Tambahkan style inactive
        ...
        // Terapkan ke tombol yang diklik
